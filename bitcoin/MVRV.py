...
df_btc['Log10 MVRV'] = np.log(df_btc['CapMVRVCur'])
...
dr_fcast = pd.date_range(df_btc['time'].iloc[0], '2030-01-01')
...
ax.semilogy(df_btc['time'], df_btc['CapMVRVCur'], color='black', zorder=3, label='BTC - MVRV')
ax.fill_between(dr_fcast, sup_lb, sup_ub, color='green', alpha=0.5, label='Support Band', zorder=2)
ax.fill_between(dr_fcast, res_lb, res_ub, color='red', alpha=0.5, label='Resistance Band', zorder=2)
...
plt.xlim(df_btc['time'].iloc[0], df_btc['time'].iloc[-1] + pd.tseries.offsets.DateOffset(years=5))
plt.ylim(0.3, 9)
