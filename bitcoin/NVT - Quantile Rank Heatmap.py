...
df_btc = CoinMetricsClient().get_asset_metrics(assets='BTC', metrics=['TxCnt', 'CapMrktCurUSD'], page_size=10000).to_dataframe()
df_btc.dropna(inplace=True)
df_btc['time'] = df_btc['time'].dt.tz_localize(None)
df_btc['NVT'] = df_btc['CapMrktCurUSD'] / df_btc['TxCnt']
df_btc['Log10_NVT'] = np.log10(df_btc['NVT'])
print(df_btc)

X = sm.add_constant(df_btc.index.astype(float).values.reshape(-1, 1))
y = df_btc['Log10_NVT'].astype(float).values
...
df_btc['Quantile Rank'] = (df_btc['Log10_NVT'].to_numpy()[:, None] > df_preds.to_numpy()).mean(axis=1)
...
ax[0].scatter(df_btc['time'], df_btc['NVT'], c=df_btc['Quantile Rank'], s=3, cmap='RdYlGn_r', zorder=3)
