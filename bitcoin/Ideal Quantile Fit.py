...
def GetIdealQuantile(y, X, q_start=0.2, q_end=0.6, step=0.01):
    ...
    for q in np.arange(q_start, q_end + step, step):
        ...
        kde_xvals = np.linspace(resid.min(), resid.max(), len(y))
...
q = GetIdealQuantile(y, X, 0.24, 0.26, 0.0001)
#q = 0.2493

model = sm.QuantReg(y, X).fit(q)
log10_fit = model.predict(X)
resid = df_btc['Log10_Price'] - log10_fit
...
ax.semilogy(df_btc['time'], df_btc['PriceUSD'], label='Bitcoin Closing Price - USD', zorder=3)
ax.semilogy(dr_forecast, np.power(10, log10_forecast), label=f'Q{q*100:.3f} Fit')
ax.semilogy(dr_forecast, np.power(10, log10_forecast + p0), color='tab:green', label='No chance of going lower')
ax.semilogy(dr_forecast, np.power(10, log10_forecast + p1), color='tab:green', linestyle='dashdot', label='1% chance of going lower')
ax.semilogy(dr_forecast, np.power(10, log10_forecast + p5), color='tab:green', linestyle='dotted', label='5% chance of going lower')
ax.semilogy(dr_forecast, np.power(10, log10_forecast + p10), color='tab:green', linestyle='dashed', label='10% chance of going lower')

ax.semilogy(dr_forecast, np.power(10, log10_forecast + p75), color='tab:red', alpha=0.5, label='25% chance of going higher')
ax.semilogy(dr_forecast, np.power(10, log10_forecast + p90), color='tab:red', linestyle='dashed', label='10% chance of going higher')
ax.semilogy(dr_forecast, np.power(10, log10_forecast + p95), color='tab:red', linestyle='dashdot', label='5% chance of going higher')
ax.semilogy(dr_forecast, np.power(10, log10_forecast + p99), color='tab:red', linestyle='dotted', label='1% chance of going higher')
ax.semilogy(dr_forecast, np.power(10, log10_forecast + p100), color='tab:red', label='No chance of going higher')
