...
btc_blockheight_data_early = pandas.read_csv(r'C:\Users\user\Desktop\Python\bitcoin\BTC_BlockHeight_Price_Early.csv', delimiter='\t')
btc_blockheight_data_early['Date'] = pandas.to_datetime(btc_blockheight_data_early['Date'])


print(btc_price_data_early)
print(btc_blockheight_data_early)


btc_blockheight_data_early['Price'] = btc_price_data_early['Price']
...
for blk_hgt_mult in range(1, 11):
    pyplot.axvline(210000 * blk_hgt_mult, color='black', linestyle='--', linewidth=0.75)

ax.loglog(btc_blockheight_data_early['BlockHeight'], btc_blockheight_data_early['Price'], color='steelblue',  basex=10, basey=10)
ax.loglog(btc_price_data['BlockHeight'], btc_price_data['Price'], color='steelblue',  basex=10, basey=10)
#ax.set_xscale('log', bas)

ax.xaxis.set_major_locator(ticker.LogLocator(base=10, numticks=500))
ax.xaxis.set_major_formatter(ticker.StrMethodFormatter('{x:,.0f}'))

ax.yaxis.set_major_locator(ticker.LogLocator(base=10, numticks=500))
...
pyplot.xlim(10000,10000000)
pyplot.ylim(10 ** -4, 10000000)
