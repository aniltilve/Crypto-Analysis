...
df_btc['Monthly Returns'] = df_btc['PriceUSD'].pct_change(30)
...
popt, _ = curve_fit(exp_decay, df_peaks.index, df_peaks['Monthly Returns'], p0=[9, -0.0005, 0.1])
print(popt)
fit = exp_decay(df_btc.index, *popt)
resid = df_peaks['Monthly Returns'] - exp_decay(df_peaks.index, *popt)
...
ax.plot(df_btc['time'], fit, zorder=4, color='tab:red')
ax.fill_between(df_btc['time'], fit + lb, fit + ub, color='tab:red', alpha=0.5)
ax.plot(df_btc['time'], np.zeros(len(df_btc)), zorder=4, color='black')
