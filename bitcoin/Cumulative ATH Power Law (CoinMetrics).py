...
#df_btc['Date Index'] = (df_btc['time']- pd.to_datetime('2009-01-03')).dt.days
df_btc['Date Index'] = (df_btc['time']- df_btc['time'].min()).dt.days + 1
df_btc['CumMax'] = df_btc['PriceUSD'].cummax()
#df_btc['New ATH'] = df_btc['CumMax'] == df_btc['PriceUSD']
#df_btc = df_btc[df_btc['New ATH']]
...
ax.semilogy(dr_forecast, np.power(10, log10_forecast), zorder=3, label=f'Quantile Q{q*100:.17f} Fit')
ax.semilogy(dr_forecast, np.power(10, log10_forecast + p0), color='tab:green', zorder=3, label='No chance of going lower')
ax.semilogy(dr_forecast, np.power(10, log10_forecast + p1), color='tab:green', zorder=3, linestyle='dotted', label='1% chance of going lower')
...
ax.semilogy(dr_forecast, np.power(10, log10_forecast + p10), color='tab:green', zorder=3, linestyle='dashdot', label='10% chance of going lower')

ax.semilogy(dr_forecast, np.power(10, log10_forecast + p75), color='tab:red', zorder=3, linestyle=(0, (1, 10)), label='25% chance of going higher')
ax.semilogy(dr_forecast, np.power(10, log10_forecast + p90), color='tab:red', zorder=3, linestyle='dashdot', label='10% chance of going higher')
...
ax.semilogy(dr_forecast, np.power(10, log10_forecast + p99), color='tab:red', zorder=3, linestyle='dotted', label='1% chance of going higher')
ax.semilogy(dr_forecast, np.power(10, log10_forecast + p100), color='tab:red', zorder=3, label='No chance of going higher')
