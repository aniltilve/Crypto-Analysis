...
ax.plot(df_btc_adr_bal['time'], df_btc_adr_bal['AdrBalNtv10KCnt'])

ax.set_ylim(0, 150)
ax.set_xlim(df_btc_adr_bal['time'].iloc[0], df_btc_adr_bal['time'].iloc[-1])


ax_window = plt.axes([0.13, 0.01, 0.8, 0.03])
slider_window = Slider(ax_window, 'window length', 10, 10000, valinit=500, valstep=1)

sg_filter, = ax.plot(df_btc_adr_bal['time'], )

def update(val):
    window = slider_window.val
    sg_filter.set_ydata(savgol_filter(df_btc_adr_bal['AdrBalNtv10KCnt'], window, 2))
